echo '------------------sign------------------'
check_result=$(curl -H "cookie:${COOKIE}" -H 'content-type:application/json;charset=UTF-8' -d '{"token": "glados.one"}' -X POST 'https://glados.rocks/api/user/checkin' | grep -Eo '"message":"[^"]*"')
echo $check_result
...
echo '-----------------status-----------------'
status_result=$(curl -H "cookie:${COOKIE}" -X GET 'https://glados.rocks/api/user/status' | grep -Eo '"leftDays":"[^"]*"')
echo $status_result
